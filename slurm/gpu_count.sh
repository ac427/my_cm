...
gpu_jobs=$(for i in $(squeue -h -o "%i" -t "running");do scontrol show job $i | grep gpu > /dev/null; if [ $? -eq 0 ] ; then scontrol show job $i | grep "NodeList" | grep -v "ReqNode" | awk -F'=' '{print $2}'  ;fi; done | tr "\n" "\t") 
nodes_that_can_take_jobs=$(sinfo -o "%N %G" -t "idle,mixed" | grep gpu  | awk '{print $1}' )
