...
#			toaddr_cmd = "ldapsearch -LLL -x -H ldap://master.eth.cluster -b dc=cm,dc=cluster cn=" +user_pids_list[4] +"| grep mail | awk '{print $2}'"
#			toaddr=subprocess.Popen(toaddr_cmd,stdout=subprocess.PIPE,shell=True).communicate()[0]
			...
			if user_pids.poll() is None:
				...
				for item in range(0,len(user_pids_list),7):
					BODY+=', '.join(user_pids_list[0:7]) + NEWLINE
#				recipients_cmd = "ldapsearch -LLL -x -H ldap://master.eth.cluster -b dc=cm,dc=cluster cn=" +user_pids_list[4] +"| grep mail | awk '{print $2}'"
#				recipents=subprocess.Popen(recipients_cmd,stdout=subprocess.PIPE,shell=True).communicate()[0]
			msg+=BODY
